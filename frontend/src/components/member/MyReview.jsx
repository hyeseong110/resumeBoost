...
const MyReview = () => {
  ...
  const memberReviewFn = async (memberId) => {
    try {
      const res = await jwtAxios.get(
        `http://localhost:8090/review/memberReview/${memberId}`
      )
      ...
      const memberRes = await jwtAxios.get(
        `http://localhost:8090/member/memberDetail/${res.data.review[0].memberId}`
      )
          ...
          <ul>
            {reviews.map((review) => (
              <li
                key={review.id}
                onClick={() =>
                  navigate(`/member/mentorDetail/${review.mentorId}`)
                }
                className='review-click'
              >
                <div className='review-top'>
                  <div className='review-top-left'>
                    <div className='review-profile'>
                      ...
                      <span className='review-last'>
                        {review.mentorNickName}님의 리뷰
                      </span>
                    </div>
                  </div>
